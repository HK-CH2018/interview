- [1 优化http router](#1-优化http-router)
  - [1. 减少栈帧与锁的延迟调用](#1-减少栈帧与锁的延迟调用)
  - [2. 优化编译-内联/struct内存对齐](#2-优化编译-内联struct内存对齐)
  - [3 log日志打印写磁盘优化](#3-log日志打印写磁盘优化)
  - [4 优化锁的粒度](#4-优化锁的粒度)
    - [4.1 拆分大锁为小锁-分段锁](#41-拆分大锁为小锁-分段锁)
    - [4.2 读写场景下，互斥锁改为读写锁，读的允许并发读](#42-读写场景下互斥锁改为读写锁读的允许并发读)
    - [4.3 细粒度对象锁, 优化前：整个结构体一把锁](#43-细粒度对象锁-优化前整个结构体一把锁)
    - [4.4 使用sync.Map替代手动锁](#44-使用syncmap替代手动锁)
- [5 优化golang GC](#5-优化golang-gc)
- [6 高并发下如何减少 context.WithTimeout 的内存分配？](#6-高并发下如何减少-contextwithtimeout-的内存分配)
  - [6.1 1 最有效的优化手段](#61-1-最有效的优化手段)
