- [1 mvcc是什么？](#1-mvcc是什么)
- [2 mvcc解决了什么问题？](#2-mvcc解决了什么问题)
- [3 mvcc实现原理](#3-mvcc实现原理)
  - [3.1 innodb隐藏列](#31-innodb隐藏列)
  - [3.2 undo log](#32-undo-log)
  - [3.3 read view](#33-read-view)
- [4 举例子说明事务可见性是怎么工作的](#4-举例子说明事务可见性是怎么工作的)
  - [4.1 构造数据版本链](#41-构造数据版本链)
  - [4.2 read view 判断流程图（决定到底看到哪个版本）](#42-read-view-判断流程图决定到底看到哪个版本)
- [5 事务 ID 与 read view 比较口诀](#5-事务-id-与-read-view-比较口诀)
...
开始快照读时：   
先选取历史链上的第一个版本号为 99 的版本，在 read view 中进行比较：
99 在 m_ids 中所以不可见。   继续沿 roll_pointer 走到下一版本   
然后选择第二个版本链 80 在 read view 中比较：
80 < min_trx_id=90    可见   
所以事务看到的就是 trx_id=80 的版本   
# 5 事务 ID 与 read view 比较口诀
判断一个版本是否对当前事务可见：   
